...
library(WGCNA)
library(sva)
## Load normalized Illumina expression data for Cerebellum

load('../data/Normalized_data_CBL.rda')
...
save(normExpr,targets,file="../output/ENSG_Normalized_data_Cerebellum.rda")


#########PCA


pdf("../output/PCA_Cerebellum.pdf",height=20,width=24)
...
prior.plots=FALSE;  ##Check with  parametric adjustment, if red and black lines are overlapping use parametric (fast), else non-parametric (slow)
	...
	datExpr.Combat = ComBat(dat=datExpr.Combat,batch=batch,mod=mod0,par.prior= par.prior,prior.plots= prior.plots);
		...
		batch = as.numeric(factor(targets$Batch));
		mod0 = model.matrix(~1,data=batch.all);
...
pdf("../output/PCA_Cerebellum_Combat.pdf",height=20,width=24)
...
save(datExpr.Combat,targets,file="../output/CombatCorrected_Cerebellum.rda")
...
save(normExpr.reg,targets,file="../output/Regressed_data_CBL.rda")


library(WGCNA)
pdf("../output/PCA_Cerebellum_RegressionRIN.pdf",height=20,width=24)
